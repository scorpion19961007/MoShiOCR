...
- 左侧“识别文本”框支持 `Ctrl+V` 直接粘贴文字后翻译
- 识别成功后可按 `Esc` 快速隐藏界面，软件继续在后台运行
- 表格识别支持独立全局快捷键，默认 `Ctrl+F10`
- 按下表格识别快捷键会直接框选截图并执行表格 OCR
- 表格 OCR 结果以网格表格显示，复制后可直接粘贴到 Excel、WPS 或 Word
- 截图识别前会等待主窗口完全隐藏，避免窗口残影影响截图内容
- OCR 服务商可在设置中选择百度智能云或腾讯云
- 翻译会随服务商切换：百度使用百度翻译，腾讯云使用腾讯云 TMT 通用文本翻译
- 设置页会根据当前服务商只显示对应的 API 凭据，减少配置混淆
